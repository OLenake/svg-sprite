...
SVGSpriter.prototype._transformShape = function(shape, cb) {
    const tasks = [];
    const that = this;
    const createTransformTask = function(transform) {
        ...
        if ((transform[0] in that._shapeTransformers) && _.isObject(transform[1])) {
            return function() {
...
SVGSpriter.prototype.compile = function() {
    ...
    const config = _.isPlainObject(args[0]) ? this.config.filter(args.shift()) : _.clone(this.config.mode, true);
    const cb = _.isFunction(args[0]) ? args.shift() : function(error) {
        throw error;
    };
...
SVGSpriter.prototype._compile = function() {
    ...
    if (!this._queue.active && this._compileQueue.length) {
        const that = this;
        const args = this._compileQueue.shift();
        ...
        // Else
            ...
            const masterShapes = _.reject(this._shapes, shape => Boolean(shape.master)).length;
            ...
            _.reject(this._shapes, shape => Boolean(shape.master))
                .forEach(function(shape, index) { // TODO split change
                    shape.setNamespace(this._indexNamespace(index));
                }, this);

            this._layout(args[0], (error, files, data) => {
                // Add intermediate SVG files
                if (that.config.shape.dest) {
                    files.shapes = that._getShapeFiles(that.config.shape.dest);
                    that.verbose('Returning %d intermediate SVG files', files.shapes.length);
                ...
                that.info('Finished %s sprite compilation', Object.keys(data).map(mode => '«' + mode + '»').join(' + '));
...
SVGSpriter.prototype._layout = function(config, cb) {
    ...
    const createLayoutTask = function(k, m) {
        return function(_cb) {
...
SVGSpriter.prototype._getShapes = function() {
    ...
    if (!this._queue.active) {
        while (this._shapesDest.length) {
            const args = this._shapesDest.shift();
...
SVGSpriter.prototype._logStats = function(files) {
    ...
    this.info('Created ' + _.toPairs(exts).map(ext => {
