...
SVGSpriter.prototype._transformShape = function(shape, cb) {
    ...
    const createTransformTask = transform => {
        ...
        if (_.isFunction(transform[1])) {
            return () => {
                transform[1](shape, this, arguments[arguments.length - 1]);
            };
        }
        ...
        if ((transform[0] in this._shapeTransformers) && _.isObject(transform[1])) {
            return () => {
                this._shapeTransformers[transform[0]](shape, transform[1], this, arguments[arguments.length - 1]);
            };
        }
...
SVGSpriter.prototype.compile = function() {
    ...
    const cb = _.isFunction(args[0]) ? args.shift() : error => {
        throw error;
    };
    ...
    if (!this._queue.active && this._compileQueue.length) {
        ...
        // Else
            ...
            _.reject(this._shapes, shape => Boolean(shape.master))
                .forEach((shape, index) => { // TODO split change
...
SVGSpriter.prototype._logStats = function(files) {
    ...
    this.info('Created ' + _.toPairs(exts).map(ext => ext[1] + ' x ' + ext[0].substr(1)).join(', '));
