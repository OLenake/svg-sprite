...
/**
     ...
     * @type {Array}
     */
const spriteTypes = ['css', 'view', 'defs', 'symbol', 'stack'];
/**
...
class SVGSpriterConfig {
    constructor(config) {
                ...
                for (const a in align) {
                    if (_.isPlainObject(align[a]) && Object.keys(align[a]).length) {
                        ...
                        for (const tmpl in align[a]) {
                            const template = tmpl.length ? ((tmpl.indexOf('%s') >= 0) ? tmpl : ('%s' + tmpl)) : '%s';
        ...
        if (Array.isArray(transforms)) {
            transformers: for (let t = 0; t < transforms.length; ++t) {
