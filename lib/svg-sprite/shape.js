 ...
 */
const createIdGenerator = function(template) {
    /**
     * ID generator
     *
     ...
     */
    const generator = function(name/*, file*/) {
...
class SVGShape {
    ...
    _initSVG() {
        ...
        const svgStart = this.svg.current.match(/<svg(?:\s+[a-z0-9-:]+=("|').*?\1)*\s*(?:(\/)|(>[^]*<\/svg))>/i);
        if (!svgStart) {
        ...
        const entityRegExp = /<!ENTITY\s+([^\s]+)\s+("|')(.+)(?:\2)>/i;
        let entityStart = 0;
        let entities = 0;
        const entityMap = {};
        let entity;
        ...
        for (let c = 0, children = this.dom.documentElement.childNodes, cl = children.length, meta = { title: 'title', description: 'desc' }; c < cl; ++c) {
            for (const m in meta) {
    ...
    setViewbox(x, y, width, height) {
        if (Array.isArray(x)) {
            this.viewBox = x.map(n => parseFloat(n));
    ...
    _setDimensions(cb) {
        ...
        const includePadding = (['padding', 'icon'].indexOf(this.config.spacing.box) >= 0);
     ...
     */
    _replaceIdAndClassnameReferences(str, substIds, substClassnames, selectors) {
     ...
     */
    _replaceIdAndClassnameReferencesInCssSelectors(str, rules, substIds, substClassnames) {
        let css = '';
        ...
        rules.forEach(rule => {
            ...
            // Regular selector
                ...
                const ids = [];
                let classnames = [];
                const classnameFilter = function(classname) {
                    if (('.' + classname) in substClassnames) {
                        classnames.push(classname);
                    }
                };

                const idOrClassSubstitution = function(sel) {
                    ...
                    if (('classNames' in sel.rule) && (substClassnames !== null) && Array.isArray(sel.rule.classNames)) {
                        sel.rule.classNames.forEach(classname => classnameFilter(classname));
                    }
                };

                // If there are multiple subselectors, substitute all of them
                if ('selectors' in sel) {
                    sel.selectors.forEach(selector => idOrClassSubstitution(selector));
                }
