...
module.exports = function(shape, config, spriter, cb) {
    ...
    config.plugins.push({ removeXMLProcInst: Boolean(spriter.config.svg.xmlDeclaration) }, { removeDoctype: Boolean(spriter.config.svg.doctypeDeclaration) });

    const svg = shape.getSVG(false);
    const svgLength = svg.length;
    const svgoInstance = new SVGO(config);

    svgoInstance.optimize(svg).then(result => {
        shape.setSVG(result.data);
        let optSVGLength = null;
        for (let t = 0, tl = spriter.config.log.transports.length; t < tl; ++t) {
