 ...
 */
class SVGSpriteCss extends SVGSpriteBase {
    constructor(spriter, config, data, key) {
        super(spriter, config, data, SVGSpriteBase.prototype.MODE_CSS);
        this.tmpl = 'css';

        this.LAYOUT_VERTICAL = 'vertical';
        this.LAYOUT_HORIZONTAL = 'horizontal';
        this.LAYOUT_DIAGONAL = 'diagonal';
        this.LAYOUT_PACKED = 'packed';
    }
    ...
    _layout() {
        ...
        if (!this._displaceable) {
            this.data.shapes = _.reject(this.data.shapes, shape => Boolean(shape.master));
        }
     ...
     */
    _addShapeToSimpleCssSprite(shape, needsRegular, index, position) {
     ...
     * @return {SVGSpriteCss}           Self reference
     */
    _layoutBinPacked(pseudoShapeMap) {
     ...
     */
    _addShapeToCSSSprite(shape, needsRegular, index, position, rootAttributes, positionX, positionY) {
    ...
    _buildSVG(xmlDeclaration, doctypeDeclaration) {
        ...
        const svg = new SVGSprite(this.declaration(this.config.svg.xmlDeclaration, xmlDeclaration), this.declaration(this.config.svg.doctypeDeclaration, doctypeDeclaration), rootAttributes, true, this.config.svg.transform);
