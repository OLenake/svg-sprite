...
class SVGSpriteLayouter {
    constructor(spriter, config) {
        ...
        this._spriter._shapes.forEach((shape, index) => {
            ...
            const { padding } = shape.config.spacing;
    ...
    layout(files, key, mode, cb) {
        ...
        const config = _.merge(_.merge(_.clone(defaultConfig[mode], true), { svg: this._spriter.config.svg }), this.config[key] || {});
