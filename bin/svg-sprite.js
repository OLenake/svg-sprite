...
function addOption(name, option) {
    ...
    if ('description' in option) {
        ...
        if ('default' in option) {
            const template = (name.substr(-9) === '-template');
...
try {
    const options = yaml.safeLoad(fs.readFileSync(path.resolve(__dirname, 'config.yaml'), 'utf8'));
...
_.reduce(argv._, (f, g) => [...f, ...glob.sync(g)], [])
    .forEach(file => {
        let basename = file;
